...
@tool(
    ...
    description=(
        ...
        "This is the verification surface for any change to the chain. "
        "After `pause_dag`, a successful pause reads as `paused=true` "
        "with children still in `waiting` — note that this is a pause "
        "that WORKED and a chain that is still stuck; the pause "
        "self-expires. After `replay_dlq_by_ids` on a dead-lettered "
        "root, a successful un-stick reads as no node in `dead_letter` "
        "and the descendants that were `waiting` promoted. "
