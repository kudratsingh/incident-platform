...
async def test_page_below_one_is_rejected(
    ...
    """A page below 1 makes `(page - 1) * page_size` negative, and Postgres refuses a
    negative OFFSET with a 500 rather than a 422 — the caller's bad input rendered as
    the server's fault."""
...
async def test_page_size_outside_bounds_is_rejected(
    ...
    """`page_size` needs a floor (0 and below are meaningless as a LIMIT) and a ceiling (an
    unbounded LIMIT is a one-request memory exhaustion)."""
...
async def test_boolean_is_not_a_tenant_limit(
    ...
    """`isinstance(True, int)` is `True` in Python, so a JSON `true` passed the hand-rolled
    guard and silently became a rate limit of 1 — a tenant throttled to one request a
    minute by a typo."""
...
async def test_non_integer_tenant_limits_are_rejected(
    ...
    """Negatives, strings, floats, explicit null, and anything wider than the `INTEGER`
    column are all refused before they reach the UPDATE."""
    resp = await client.patch(
        f"/api/v1/admin/tenants/{admin_user.tenant_id}",
        json={field: value},
        headers=admin_headers,
...
async def test_unknown_tenant_limit_field_is_rejected(
    ...
    """A misspelled field used to be dropped on the floor and answered 200, so an operator
    who thought they had raised a quota had not."""
    resp = await client.patch(
        f"/api/v1/admin/tenants/{admin_user.tenant_id}",
...
async def test_valid_tenant_limits_still_apply(
    ...
    """Zero stays legal — it is how an operator disables a check — and a partial body still
    leaves the untouched field alone."""
...
async def test_tenant_limits_validation_precedes_the_lookup(
    ...
    """Body validation runs before the tenant is fetched, so a bad body against an unknown
    tenant is still a 422 and never a 404 — the caller learns about every problem with
