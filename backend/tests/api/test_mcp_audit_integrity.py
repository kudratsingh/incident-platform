...
def _client_for(session: Any) -> AsyncClient:
    """An MCP client whose `get_db` yields `session` — a real one, or a proxy that behaves
    like Postgres under failure."""
    app = create_mcp_app()
...
async def test_oversized_request_id_still_writes_the_audit_row(
    ...
    """The finding's sharp end: a 4KB header used to leave the tool run and committed with
    no audit row."""
...
async def test_a_usable_request_id_is_recorded_verbatim(
    ...
    """The correlation the header exists for still works — this fix must not cost the agent
    its ability to tie a call to its own trace."""
...
async def test_an_unwritable_audit_row_fails_the_call(
    ...
    """The general rule behind R2-51: if the row cannot be written, the request fails."""
...
async def test_deploy_history_fallback_still_writes_its_audit_row(
    ...
    """`deploy_markers` absent — the exact condition the env fallback exists for."""
