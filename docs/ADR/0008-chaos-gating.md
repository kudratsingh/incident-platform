...
While correcting them: the set is **nine** tools, not the five that `docker-compose.yml` claimed — `kill_consumer`, `poison_message`, `saturate_redis`, `inject_latency`, `bad_deploy`, `create_bad_data_job`, `create_stale_cache`, `create_stuck_dag`, `seed_dlq_messages`. The compose comment is pinned to the registry by `backend/tests/unit/test_docs_chaos_surface_drift.py`.

**Amendment (2026-09-08, WO-R2-166):** the set is **ten**, not nine — `create_mislabeled_dlq_job` joins it. Left as an amendment rather than an edit to the paragraph above, because that paragraph is dated and the count has now moved twice; the enumeration in `docker-compose.yml` is the copy the drift test pins, so read it there rather than from either sentence here. The new hook is worth a line in this ADR on its own account: it is the first chaos tool whose output is *deliberately misleading* rather than merely destructive, writing a dead-letter row whose `remediation_hint` contradicts its own error text. That is a fourth reason the `CHAOS_ENABLED` gate matters — a production stack must not be able to manufacture a row that misdirects an operator — and it is why the hook carries a second, in-band gate of its own: a required `mislabel: true` argument with no default, on the same defence-in-depth reasoning as the triple gate this ADR records. The Pointers entry for `backend/app/config.py` still reads "to be added in Wave 1 PR #4"; the assertion shipped, and the parenthetical is left as a dated note rather than rewritten.

**Amendment (2026-09-15, WO-R3-187 / owner decision O-4): the seed grant moves after all, to a second principal.** The dated section above decided "the documentation moves, the seeder does not" and leaned on the seeder's never-narrows property. Both halves of that are now qualified, and the reason is a leak on a surface that section was not about.

`list_audit_events` and `get_trace` withhold the `chaos.` audit stream from any principal that does not hold `chaos:invoke` (2026-09-15 amendment to [ADR 0012](0012-the-lab-is-invisible-to-the-agent.md)). A predicate keyed on the scope does nothing while the principal under test holds the scope — so the seed grant had to move off the agent, and the reasoning above ("the grant is deliberate and load-bearing") now points at a different account rather than a different answer:

- `scripts/seed_incident_commander.py` mints two service accounts. `incident-commander` holds the two read scopes plus `actions:execute`; `incident-commander-chaos` holds the read scopes plus `chaos:invoke` and is what the eval runner uses to seed, verify and reset a world. The class of scenario the grant exists for is unaffected — it is fired by the evaluator, which is who was firing it all along.
- The seeder does now narrow, exactly once and only on the agent account: `chaos:invoke` is removed if found, and said so on stderr. D-01's rule is that a grant is never dropped *silently*, and `SA_REPLACE_SCOPES=1` remains the escape hatch for everything else. `SA_SCOPES` naming `chaos:invoke` is refused outright rather than quietly narrowed, and points at `SA_CHAOS_SCOPES`.
- Nothing about the three gates changes. Gate 1 (`CHAOS_ENABLED`) is still what protects production, gate 2's scope check is still the only thing that admits a call, and the v0.5.0 API-grant gate is untouched. Gate 2's own sentence — "the seed `incident-commander` principal does not have it" — is true again as of this change, but read it the way the paragraph above says to: the authority is `SELECT name, scopes FROM service_accounts`, not this document.
