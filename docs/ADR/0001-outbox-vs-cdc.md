...
- **`CANCELLED` is exempt.** It is terminal, but the platform has no `job.cancelled` topic to announce it on, so there is no event to write in the transaction. It has two writers. `SagaCoordinator._handle_failure` cancels steps of a saga it is already settling, so the saga side stays coherent. The dependency cascade added by [ADR 0022](0022-promotable-only-resume-sweep-and-dependency-cascade.md) §3 cancels non-saga `WAITING` children of a stranded parent, and has no such coordinator behind it — so for those rows nothing reconciles anything, and the read model keeps their ids in their previous status set with no second chance. Either way the read model goes stale; the cascade widens the blast radius from saga steps to plain DAG children. Adding the topic is a schema-registry entry plus four consumers; tracked in [`docs/ROADMAP.md`](../ROADMAP.md), not smuggled in here.
