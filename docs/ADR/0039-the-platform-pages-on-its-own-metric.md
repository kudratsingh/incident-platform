...
- **One contract delta.** `get_consumer_lag`'s description had "every ~60s", "90s TTL", "up to a minute stale" and "up to 15 of them" in it — statements that were about to be false on the only stack anybody watches. The description now says the interval is **deployment-configured** and points at `age_seconds` and the gaps between `recent_samples`. `tools/list` moves: 40 tools before and after, no name, scope, field, `$defs` entry or `required` list changes, and exactly one entry differs — `get_consumer_lag`'s `description` (3,444 → 3,708 chars) and the `source` / `age_seconds` / `recent_samples` field descriptions in its `outputSchema`. Hash `31183161…` → `cb95185f…`. The commander re-pins and reblesses; the *reason* it is not avoidable is CLAUDE.md's own rule that a description which does not match the behaviour behind it is a functional defect that fails silently with a confident-looking answer.
