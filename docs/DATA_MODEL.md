...
| `max_attempts` | Integer (default `MAX_JOB_ATTEMPTS`, itself 3) | Per-job cap on the number of **runs** — the original plus its retries; see [The attempt budget, in words](#the-attempt-budget-in-words) below. Stamped at INSERT from the setting rather than from a literal, so the knob reaches every writer — REST creation, chaos hooks, eval seeds, saga steps (WO-R2-76). A caller may still name a different ceiling for one job; the saga coordinator does. Changing the setting does **not** re-stamp existing rows: the dispatcher reads the ceiling off the row, so jobs created before the change keep the budget they were admitted with. Called `max_retries` until Alembic `f1c7b93a4d26` renamed it (WO-R2-172) — a rename only: same type, same values, same arithmetic. |
| `dead_lettered_by` | String(32) NULLABLE | Which mechanism forced the dead-letter, when it was not the default one — `llm_retry_policy` is the only value today. NULL = retries simply ran out (or no processor was registered, or the dispatcher's safety net fired), so the admin DLQ tab can badge the LLM rows without inferring it from `retry_count < max_attempts`, which mislabelled every compensation job. Reset to NULL on admin Replay. Added in Alembic `c9a3e5d70b12`; not backfilled, so pre-migration rows read NULL. |
...
| `fenced_at` | TIMESTAMPTZ NULLABLE | When an operator last fenced this row with `mark_dlq_permanent`. Written on **every** mark, including a mark of a row that was already `human_required` — a re-fence is still an operator act, and before WO-R2-158 that call took an `already_marked` early return and wrote nothing at all, not even an audit row, so an agent that fenced a pre-classified row and one that skipped the step left identical worlds. This is the field a caller *verifies* a fence on: `remediation_hint` cannot do it, because `human_required` is the same value from LLM triage and from a deliberate fence. Clock: the platform process's own, aware UTC, at the moment of the call — a different clock from `completed_at`/`dead_lettered_at` (when the job died) and `created_at` (when it was submitted). NULL = nobody has fenced this row. Emitted by `list_dlq_messages` and in the `mark_dlq_permanent` response. Episode-scoped like the two columns above: reset to NULL on Replay. Added in Alembic `2821ef11e2d2`; not backfilled — historical fences are in `audit_logs.action = 'job.marked_permanent'`, and inventing a per-row timestamp from them would put a fabricated value in a column whose whole purpose is to be trusted. |
| `fenced_by` | String(64) NULLABLE | Which principal raised the fence, as `"{principal_type}:{principal_id}"` — e.g. `service_account:0f9a…`. Self-describing rather than a bare UUID because the same id space is `users.id` or `service_accounts.id` depending on the discriminator, and a column that cannot say which is exactly the shape [ADR 0007](ADR/0007-machine-principal-scope-model.md) rejected for `audit_logs`. No FK, for the same reason `audit_logs.principal_id` has none: the fence record has to outlive the principal. No index — it is a forensic read reached through the job, and the audit trail (joined on `audit_logs.resource_id`) stays the queryable record of who did what. NULL exactly when `fenced_at` is NULL, including after a Replay clears both. Added in Alembic `2821ef11e2d2`. |
...
### The attempt budget, in words

`max_attempts` counts **runs**, not retries. The dispatcher retries while
`retry_count < max_attempts`, so a row carrying the default 3 is executed three
times — the original run plus two retries — and dead-letters when the third one
fails:

| `retry_count` on entry | The run that just failed | What the dispatcher writes |
|---|---|---|
| 0 | attempt 1 of 3 — two runs left | `pending`, re-queued after the backoff |
| 1 | attempt 2 of 3 — one run left | `pending`, re-queued after the backoff |
| 2 | attempt 3 of 3 — the budget is spent | `dead_letter`, "Job exhausted after 3 attempts" |

So 3 attempts buys 2 retries, and a ceiling of 1 means one run and no retry at
all. `retry_count` is the number of runs *completed*, which is why the
dispatcher's own message reads `attempt 3 of 3` on the last one.

The column was called `max_retries` until WO-R2-172, and that name promised one
run more than the platform has ever given: a reader who set it to 3 expecting
three retries got three runs. The user's ruling on 2026-09-08 was to fix the
name and keep the arithmetic — nothing in the table above changed behaviour,
only what the field is called. The word for the count is **attempt**, and it is
the same word everywhere: the retry message, the exhaustion message, the admin
table header, and the lab's seeded error texts.
...
| `NULL` | Nothing has classified this failure, and the text must not invite a replay | worker exited without recording an outcome; also a bad CSV row nobody has triaged, and a schema violation nobody has triaged |
...
**The `NULL` row's rule is asymmetric** (WO-R2-158). It used to read "the text must not classify it either", which was too broad in one direction and made a pair the escalation drill needs representable nowhere: an *unclassified* row whose error text a human can read and act on. A hint is a classification; an error text is the symptom the failing code recorded, and "nobody has classified this" does not disagree with "the symptom is a bad row in the stored payload" — that text is the evidence a triage pass would read to *reach* `human_required`, and it points away from a replay, which is where a null hint already sits. It is also the normal state of an organically dead-lettered job here, because LLM triage is off by default. What a null hint may never carry is a text that says a replay (now, or after a wait) is the remedy — that is a routing claim the missing classification never made — and it may never carry a `job_triages` row at all, because that block *is* a classification. `coherence_violations` screens the first and `triage_violations` the second. `create_bad_data_job` pins the unclassified bad-data story by key for `dlq_human_required_escalates`, where the agent must read the error, fence the row itself, and escalate.

`coherence_violations()` in that module is the rule as code, and `tests/unit/test_dlq_text_coherence.py` walks every writer's pairs through it — `seed_dlq_messages`, `create_stuck_dag`, `poison_message` (both of its declarable hints), `create_bad_data_job` (both of its) and the four seeded rows. The API tier asserts the same thing on rows read back over the wire, so a hook that stops consulting the table fails rather than quietly drifting.

**One writer is exempt, by name, and only one** (WO-R2-166). `create_mislabeled_dlq_job` writes the lab's single sanctioned incoherent pair: hint `replay_safe`, text a permanent bad-data fault. That is the fixture, not a defect — a `remediation_hint` can be wrong in production (triage, an operator or a backfill can all write the wrong category, and nothing downstream re-derives it from the error text), so an agent that trusts the column over the evidence contradicting it has to be measurable. Three things keep the exception from becoming a loophole: the pair is declared outside `DLQ_FAILURE_STORIES` and reachable only through `sanctioned_incoherent_story()`, so no writer can resolve it from a hint; its text is the CSV bad-row one rather than the `SchemaValidationError` string from run `efdc3b2a9864`, so the narrow promise that the exact pair which shipped is never written again stays absolute; and the test suite asserts the screen still *flags* this row. The hook requires an explicit `mislabel: true` and its description says in plain words that the row is a lie.

Two consequences worth knowing before editing a lab text:

- **`poison_message`'s DLQ row is not replay-safe, and its text is the schema violation it really sends to Kafka.** That row is a stand-in — real consumers log-and-drop schema errors instead of dead-lettering them — but a stand-in that lies is worse than none. This got fixed twice. WO-R2-146 moved the *text* to a transient one so it would agree with the `replay_safe` hint the hook had always stamped; that passed the coherence screen and was still false, because the hook injects a schema violation and nothing else. WO-R2-166 moved the **hint** instead: the row now says `SchemaValidationError`, and its `remediation_hint` is `NULL` by default (a freshly poisoned message has been classified by nobody, because triage is off) or `human_required` on request. `replay_safe` cannot be asked for on any argument. The Kafka half stays observable in the consumer's logs and in `get_consumer_lag`; the row still names its topic and the hook. Rule this leaves behind: when a fixture's hint and text disagree, check which half describes what the hook actually did before deciding which one to move.
