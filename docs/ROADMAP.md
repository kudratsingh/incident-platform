...
| ~~A `job.cancelled` topic~~ | — | **Shipped (WO-R2-113).** `job.cancelled` is a first-class lifecycle topic: schema file, `kafka_topic_job_cancelled` setting, and all four consumers taught (read model, event log, SSE, audit). The open question in this entry — whether `_TRACKED_STATUSES` gains a `cancelled` set or folds into `dead_letter` — was answered with its own set, because folding would have made a cancellation indistinguishable from a failure on the overview tab, and because `_move` only removes an id from statuses it tracks. Both writers emit: `update_status` and the dependency cascade, which turned out not to go through it. |
