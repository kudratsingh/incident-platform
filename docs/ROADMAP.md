...
| A `job.cancelled` topic | S | `CANCELLED` is the one terminal status with no event, so `JobRepository.update_status` writes it silently — the only hole left in the "terminal state and terminal event are one write" invariant (see the addendum on [ADR 0001](ADR/0001-outbox-vs-cdc.md)). Its single writer is `SagaCoordinator._handle_failure`, cancelling downstream steps of a saga it is already settling, so saga state stays coherent; what goes stale is the CQRS read model, which leaves those ids in their previous status set forever, and the SSE stream, which never closes even though `progress.TERMINAL_STATUSES` already lists `cancelled`. Cost is a schema file, a `kafka_topic_job_cancelled` setting, and four consumers to teach (read model, event log, SSE, audit) — plus a decision on whether `_TRACKED_STATUSES` gains a `cancelled` set or folds it into `dead_letter`. Do it when a saga rollback first shows up as a wrong number on the overview tab. |
