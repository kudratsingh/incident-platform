...
| Dedicated worker deployable (Phase 8) | M | `worker_loop` runs in every API replica's lifespan today, which is why the outbox relay needs an advisory-lock leader gate to stay single-writer ([ADR 0020](ADR/0020-outbox-relay-single-writer.md)). Splitting the consumers and background loops into their own ECS service — sized to one task, separate from API autoscaling — subsumes the gate as the structural answer. Keep the gate anyway until that service genuinely deploys without overlap: blue/green, also a Phase 8 item, by definition runs two of them. |
| Outbox → CDC migration via Debezium | L | Replace `_outbox_relay_loop` with a Debezium source connector against the `outbox_events` table. Easier than full CDC because we keep the application-controlled event taxonomy. Sunset the polling relay at the end — which retires the leader gate with it ([ADR 0020](ADR/0020-outbox-relay-single-writer.md)). |
...
| A `job.cancelled` topic | S | `CANCELLED` is the one terminal status with no event, so `JobRepository.update_status` writes it silently — the only hole left in the "terminal state and terminal event are one write" invariant (see the addendum on [ADR 0001](ADR/0001-outbox-vs-cdc.md)). It now has two writers. `SagaCoordinator._handle_failure` cancels downstream steps of a saga it is already settling, so saga state stays coherent. The dependency cascade ([ADR 0022](ADR/0022-promotable-only-resume-sweep-and-dependency-cascade.md) §3) cancels non-saga `WAITING` children of a `DEAD_LETTER`/`CANCELLED` parent, with no coordinator behind them at all. What goes stale is the CQRS read model, which leaves those ids in their previous status set forever, and the SSE stream, which never closes even though `progress.TERMINAL_STATUSES` already lists `cancelled`. Cost is a schema file, a `kafka_topic_job_cancelled` setting, and four consumers to teach (read model, event log, SSE, audit) — plus a decision on whether `_TRACKED_STATUSES` gains a `cancelled` set or folds it into `dead_letter`. Do it when a saga rollback first shows up as a wrong number on the overview tab — or sooner: the dependency cascade means plain DAG failures now produce silently-cancelled rows too, so the trigger no longer bounds the exposure the way it did when sagas were the only source. |
