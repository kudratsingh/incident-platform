...
| `kafka:consumer_lag:worker-dispatcher` | string (int) | `dispatcher.py` `_metrics_loop` | `backpressure.py` `check_backpressure` | **three metrics passes** — `core/consumer_lag.lag_value_ttl_seconds`, so 180s at the default 60s interval and 15s at the demo stack's 5s (WO-R3-338; a fixed 90s until then) | Yes — falls back to "no backpressure" |
| `kafka:consumer_lag:{group}:samples` | string (JSON list, newest first, `{lag, measured_at}` — 15 minutes of history at one per pass, so 15 entries at a 60s interval and 180 at 5s, under an absolute 240 guard) | `core/consumer_lag.record_lag_sample`, called by `dispatcher.py` `_record_lag_sample` from the same loop pass that writes the value above. Keyed by group; only `worker-dispatcher` has a writer | `GET /admin/consumer-lag`, which serves the whole window for the operator chart, and the `get_consumer_lag` MCP tool, which returns the newest **15** of it (`LAG_SAMPLES_AGENT_CAP` — the agent pays for each sample in context on every read, so its reading's size must not move with the tick) | **1080s** (the 900s window plus 180s), refreshed on every write | Yes — the reader reports an absent window as no history, never as a flat lag |
| `outbox:relay:last_tick` | string (ISO-8601 UTC timestamp) | `dispatcher.py` `_outbox_relay_tick` via `core/outbox_heartbeat.record_relay_tick` — every pass the relay completes, leader only, whether or not anything was delivered | `get_outbox_status` MCP tool (`relay_last_tick_at` / `relay_heartbeat_age_s`) | 24h, refreshed on every pass | Yes — the tool reports an absent record as unknown with the reason, never as an age ([ADR 0028](ADR/0028-outbox-relay-heartbeat-and-delivery-reading.md)) |
| `breaker:state:{name}` | string (JSON: state, failure count, threshold, recovery timeout, last state change, last failure + its reason class, recorded-at) | `core/circuit_breaker.py` via `core/breaker_state.record_registered_breakers` / `publish_breaker_state` — the process that owns the breaker, on every state change and at most once a minute while calls flow | `get_circuit_breakers` MCP tool | 24h, refreshed on every state change | Yes — the tool reports an absent record as unknown with the reason, never as a closed breaker ([ADR 0030](ADR/0030-breaker-state-is-published-and-a-reading-is-never-invented.md)). `make eval-reset` rewrites each record *closed* with the failure fields null rather than deleting it, for that same reason, and raises the signal below so the owning registry forgets too ([ADR 0036](ADR/0036-the-reset-closes-a-breaker-and-a-registry-it-cannot-restart-honours-it.md)) |
| `breaker:reset:at` | string (ISO-8601 UTC timestamp) | `scripts/reset_eval_state.py` via `core/breaker_state.publish_breaker_reset` — once per `make eval-reset`, before it rewrites a single state record above | every `CircuitBreaker` in every process, through `core/breaker_state.read_breaker_reset_at`: when the signal is newer than the breaker's own last failure it clears itself to its boot state, so a fault the lab already cleared stops being republished and stops refusing calls without a restart | 24h | Yes — an absent, unparseable or unreachable signal is read as "no reset", so a breaker keeps the state it has ([ADR 0036](ADR/0036-the-reset-closes-a-breaker-and-a-registry-it-cannot-restart-honours-it.md)) |
| `pool:state:{process}` | string (JSON: pool size, connections checked out, overflow, overflow ceiling, checkout timeouts in the last minute, written-at) | `core/pool_state.publish_pool_state`, from the per-process publisher task started in each process's lifespan (`app/main.py` for `api_worker`, `app/mcp/standalone.py` for `mcp`) — every 10s, in every environment, not gated on `CHAOS_ENABLED` and not pausable by `pause_control_loop` | `get_postgres_health` MCP tool, as the `pools` group beside the flat `pool_*` fields it does not change | 60s (six cadences), rewritten on every pass | Yes — the tool reports an empty group with `pool_gauges_unknown_reason` set, never as "every pool is fine" ([ADR 0033](ADR/0033-each-process-publishes-its-own-pool-gauge.md)). A short TTL on purpose: a process that stops publishing has to drop out of the listing rather than freeze at its last healthy number, which is the opposite trade-off from `breaker:state:*` above. Outside `chaos:*`, so the eval reset does not carry it — and does not need to, because nothing here is state a scenario set |
...
These namespaces are written by the incident-commander MCP tools and the eval tooling, not by the platform's own request/worker paths. The `chaos:*` tools are registered only when `CHAOS_ENABLED=true` (ADR 0008); the rows marked **eval-only** are seeded by `scripts/seed_eval_fixtures.py` and never appear outside an eval stack. `scripts/reset_eval_state.py` clears residue between scenarios: a `chaos:*` scan-and-delete, the `jobs:dlq_replay_delayed` + `jobs:dlq_replay_inflight` ZSETs, every `dag:paused:*` flag, the recorded consumer-lag sample window, and a re-seed of the fixture keys.

**One fault can need two of these keys at once, and the single `chaos:*` scan is what makes that safe** (WO-R3-213). Stranding a `WAITING` child — the Family C world, where the correct answer is to escalate — takes `chaos:kill:dependency-resolver` *and* `chaos:pause:resume_unblocked_waiting` together: killing the consumer group alone leaves the resume sweep to promote the child within about ten seconds, by design. Only the **pause's** TTL heals that world, because the parent's `job.completed` has already been consumed and the resolver coming back has nothing left to deliver — so a teardown that cleared one key and not the other would leave a permanently stranded child. Both are under `chaos:`, so the one scan reaches both; the property is asserted of that exact pair in `test_pause_control_loop.py`, and the reasoning is [ADR 0027](ADR/0027-control-loop-pause-closed-enum.md)'s 2026-09-17 amendment. A sticky kill is the same shape for a different reason (WO-R3-225): `chaos:kill:{group}` is the flag the platform reads and `chaos:kill_sticky:{group}` is the window that keeps putting it back, so a teardown that took only the flag would be undone on the next poll — the one scan takes both, which is what makes the fault end on its deadline instead of outliving the run.

Any key under `cache:` / `jobs:cache:` / `kafka:consumer_lag:` / `read_model:` — the same prefixes `invalidate_cache_key` may delete — is additionally observable through the `get_cache_key_info` MCP read tool (`telemetry:read`), which reports existence, TTL, type, size and the record check below, but never the value. That is the read half of the stale-cache remediation loop: check the key before invalidating, confirm the delete after.

**Shape alone could not tell a stale entry from a current one, so the reading now says whether the records are still there** (WO-R3-267). `exists` / `type` / `ttl_seconds` / `size` are identical for a hot-set entry that matches the database and one that does not — both are strings with a TTL, a few bytes apart — and `get_redis_health` only ever offered a Redis-wide miss ratio, which is about the instance and not about the key. An alert naming a stale key was therefore making a claim no sensor on this platform could support, and two paid live runs of `remediate_stale_cache_success` split on byte-identical readings: one acted, one escalated.

The evidence added is the one fact the platform can measure about any entry in these namespaces without reaching for a second system: every one of them is a copy of job records Postgres holds, so the platform can ask whether those records are still there. `records_referenced` is how many job records the entry is meant to name — the items in its stored list, or the single job its key names (`cache:job:{tenant_id}:{job_id}` is answered from the key, so the payload stays unread) — and `records_found` is how many of those `JobRepository.existing_ids_for_tenant` finds for the caller's own tenant, at call time. A current hot-set entry names three jobs and finds three; one that no longer matches the database names three and finds none. Fewer found than referenced says the copy points at records the platform does not have; equal counts say every reference resolves, which is deliberately *not* a claim that the copied fields are current, and the tool description says so. Both are null — never 0 — when the platform cannot work out what an entry refers to: a counter, a Redis collection type, a JSON object, an unparseable value, or a list longer than the tool will resolve in one call. Nothing new is written to Redis and no writer changed: the check is a query, so there is no side-car to keep consistent across a reset, and the reset's existing re-seed of the hot-set from `_dlq_specs()` is what leaves the next run opening on the healthy reading (pinned by `tests/unit/test_eval_reset.py::test_reset_leaves_the_hot_set_reading_as_healthy`).

Three alternatives were dropped, and the reasons are worth keeping. A `written_at` recorded by the cache writers: nothing in these namespaces records one, and age is not staleness without a reference — the hot-set entry is written at boot and is hours old on a long-lived stack, while a freshly written stale one is seconds old, so age reads backwards. A `source_newer` flag from the cached row's own `updated_at`: the natural check for `cache:job:` entries, except that `JobResponse` — the shape `JobCache` serialises — carries `created_at` / `started_at` / `completed_at` and no `updated_at`, and widening a REST response model to feed a cache tool is the wrong direction. Redis `OBJECT IDLETIME` / `FREQ`: idle time measures the last *access*, and this tool's own `STRLEN` is an access, so the first call would reset the signal the second call reads.

Both tools are additionally **tenant-scoped** (R2-54). The prefix allowlist answers "is this a platform cache namespace?", never "is this *your* entry?" — and `cache:job:{tenant_id}:{job_id}` is allowlisted on purpose, so without a second gate a service account in one tenant could evict another tenant's cached reads and confirm their existence, TTL and size. Withholding the payload does not close that: existence *is* the disclosure. The tenant segment now comes from the authenticated principal (`app/mcp/tools/_cache_scope.py`, shared by both tools so they cannot drift), and a key whose segment says otherwise is refused with `cache_key_forbidden` before any Redis call, identically whether or not the key exists. Key families with no tenant segment — `cache:jobs:…:hot_set`, `kafka:consumer_lag:*`, `read_model:*` — are platform-global and unaffected.

| Key pattern | Type | Writer | Reader | TTL | Eviction-safe? | Cleared by eval reset? |
|---|---|---|---|---|---|---|
| `chaos:kill:{group}` | string flag | `kill_consumer` chaos tool (`kill_key_for`); also re-written by `_rearm_sticky_kill` while the sticky marker below is live (ADR 0032) | `kafka_consumer.py` `_read_kill_state` — top of every poll and in the supervisor's kill window; consumer shuts down while set | `ttl_seconds` (default 300s); a re-arm uses `PXAT` at the sticky deadline, never a fresh TTL | Yes — chaos ends early | Yes (`chaos:*` scan) |
| `chaos:kill_sticky:{group}` | string (absolute unix deadline, 3dp) | `kill_consumer(sticky=true)` chaos tool (`sticky_kill_key_for`) | The same MGET as the row above. While this is live and the kill flag is gone, the flag is put back for what is LEFT of the window — so `restart_consumer_group` clears the flag, says so truthfully, and the group stays down anyway (ADR 0032). Unreadable value fails open; the re-arm writes only under `CHAOS_ENABLED` | `ttl_seconds` (default 300s), never extended — the deadline in the value and this TTL are two independent bounds on the same instant | Yes — the kill ends early | Yes (`chaos:*` scan) |
...
| `chaos:pause:{loop}` | string flag | `pause_control_loop` chaos tool (`pause_key_for` in `app/workers/control_loop_pause.py`); `{loop}` is one of the eleven members of `ControlLoopName`, never a Kafka consumer group | The named loop in `dispatcher.py`, once per iteration via `loop_is_paused` — it skips that iteration's work while set. Read only when `CHAOS_ENABLED=true`; fail-open, so an unreachable Redis reads as not-paused (ADR 0027) | `ttl_seconds` (default 300s, max 3600s) | Yes — the loop resumes | Yes (`chaos:*` scan) |
| `chaos:db_pool:hold` | string (connection count) | `saturate_db_pool` chaos tool (`hold_key` in `app/workers/db_pool_hold.py`) | The worker's chaos-only holder task, once a second: it holds that many sessions open with their transactions begun, clamped so `MIN_FREE_CONNECTIONS` (4) stay acquirable. This is the API/worker pool, not the MCP process's own, so the flat `pool_*` fields of a reading taken on the agent's surface do not show it — the `pools` group in the same reading does, from the `pool:state:api_worker` row above ([ADR 0033](ADR/0033-each-process-publishes-its-own-pool-gauge.md), closing ADR 0031's D1 and ADR 0030's "what this does not close") | `ttl_seconds` (default 300s, max 3600s) | Yes — every connection comes back | Yes (`chaos:*` scan) |
| `chaos:db_query:slow` | string (`{target}:{query_ms}`) | `slow_db_queries` chaos tool (`slow_query_key` in `app/workers/db_slow_query.py`) | The worker's chaos-only sleeper task, once a second: two real reads of the relation `target` names, each held open by a server-side sleep and offset by half a chunk, so `get_postgres_health`'s `longest_active_query_ms` / `active_queries_over_slow_threshold` never dip back under the threshold between statements. `target` is looked up in a closed map and never interpolated; an unknown target or an out-of-bounds chunk reads as off ([ADR 0034](ADR/0034-a-slow-query-is-manufactured-where-the-server-can-see-it.md)) | `ttl_seconds` (default 300s, max 3600s) | Yes — the queries stop | Yes (`chaos:*` scan); one query already in flight finishes first, so up to `query_ms` (max 10s) outlives the sweep |
| `chaos:downstream:bulk_api_sync` | string (`{mode}:{delay_ms}`) | `degrade_downstream` chaos tool (`downstream_flag_key` in `app/workers/async_tasks.py`) | `process_bulk_api_sync`, once per job — so a whole fan-out sees one dependency state. `fail` makes every endpoint return 503 and the shipped breaker opens at its own threshold; `slow` makes each answer late and succeed. The key chooses the injection, not the outcome: a job whose every endpoint call failed is itself failed with or without it (ADR 0031 and its 2026-09-19 amendment, WO-R3-322) | `ttl_seconds` (default 300s, max 3600s) | Yes — calls go back to normal | Yes (`chaos:*` scan); dead-lettered jobs left behind are organic DLQ rows, swept by `_sweep_nonfixture_dlq` |
...
| `cache:jobs:worker-dispatcher:hot_set` (**eval-only**) | JSON array of seeded DLQ job ids | `seed_eval_fixtures.py` `_seed_hot_set` (ids derived from `_dlq_specs()`, so every member is a job the seed creates — D-14); also the `create_stale_cache` chaos tool, whose fabricated ids match no row | The stale-cache scenario observes it via `get_cache_key_info` — including `records_referenced` / `records_found`, which separate the two writers' values — then deletes it via `invalidate_cache_key` | 24h (seed) / `ttl_seconds` default 600s (tool) | **No** — it is the one fixture written with a TTL, which is exactly what a `volatile-*` eviction policy takes first, and the TTL itself ends it on a stack up for a day; a later world then reads `exists: false` (WO-R3-310) | Restored by `reset_eval_state.py` `_reseed_hot_set`, which reports `hot_set_reseeded: 1` when it found the key gone or drifted |
| `kafka:consumer_lag:{group}` (**eval-only** except `worker-dispatcher`) | string (int) | Metrics loop writes only `worker-dispatcher` (row above); `seed_eval_fixtures.py` writes the 7 synthetic groups (`billing-consumer` … `healthy-consumer`) | `get_consumer_lag` MCP tool (any group); `check_backpressure` reads only `worker-dispatcher` | **none** on seeded groups (three passes on the real one) | Yes | Re-seeded by reset |
...
The `cache:` namespace also makes this key reachable by the `invalidate_cache_key` MCP action, whose allowlist covers `cache:` — an agent can force-refresh a single job read without any change to that tool's contract. Its read counterpart `get_cache_key_info` covers the same prefixes and reports existence / TTL / type / size, plus whether the database still holds the one job the key names — never the cached `JobResponse` payload, which is tenant data. The job id comes out of the key (`_cache_scope.job_segment`, the sibling of the tenant check), so the record check never needs to open the payload. Since R2-54 both are bounded to the calling principal's own tenant segment as well: this is the one allowlisted family that carries a tenant id, so it is the one the scope check exists for.
...
The metrics loop calls `dispatcher.consumer_lag()` once per pass and writes the result here under a TTL of three passes (so the key is always fresh-or-missing, never stale). **The pass interval is `METRICS_LOOP_INTERVAL_SECONDS` since WO-R3-338** — 60s by default, 5s in the demo stack (owner decision O-35) — and this TTL is derived from it rather than fixed, because the fixed 90s was 1.5 passes at 60s and eighteen passes of readable stale number at 5s. Three passes rather than 1.5 so one lost pass does not blank the reading on a healthy stack: an absent value opens the admission gate.
...
**The value is one undated integer, so the loop also records when it measured it** (WO-R3-254). Every pass that writes the value writes `kafka:consumer_lag:worker-dispatcher:samples` beside it: a JSON list, newest first, of `{lag, measured_at}` measurements. **Fifteen minutes of them under an 1080s TTL since WO-R3-328**, where it was five samples under the value key's own 90s. Since WO-R3-338 the window is pruned by **time** rather than by a count of passes: the count was `window / 60s`, which silently became 75 seconds of history on a fifteen-minute axis the moment the pass interval became a setting and the demo stack set it to 5s. So the span is the promise and the density follows the clock — 15 entries at 60s, 180 at 5s — with `LAG_SAMPLES_MAX_ENTRIES = 240` as an absolute guard that binds only below a 3.75s interval, and the reader bounding by that guard rather than by its own interval, because the reader is another process and one that trimmed the window to *its* setting would hide samples the writer kept. Two deliberate consequences of that split. The window is no longer fresh-or-absent with the value: the value must be fresh, because `check_backpressure` gates submissions on it and a stale number would reject work over a fault that is over, while history is *most wanted* at the moment the pass that writes it stopped — a window that expired 90 seconds later took the chart with it exactly then. And the reader can now hold samples older than the newest value; each carries its own `measured_at`, so a gap reads as a gap rather than as a flat line. Still a TTL, so a window nothing refreshes disappears instead of being read as current. The key, its cap and its TTL live in `app/core/consumer_lag.py` and the worker imports them, rather than one set of literals in the worker mirrored by another in the reader: a cap that drifted would have shortened the chart without shortening the axis. The value key keeps its exact shape and meaning; `check_backpressure` and every other reader of it are untouched. The window exists because the only reader of the value that matters here cannot wait: an agent doing three reads inside 25 seconds gets the one number the loop last wrote, which reads as "the lag is not moving" and actually means "not re-measured yet" — that misreading cost a live run on 2026-09-17. `get_consumer_lag` returns the window as `recent_samples`, plus `measured_at` and `age_seconds` for the current number, so the trend is answerable from a single call. It never invents either: with no window recorded, `recent_samples` is empty and `measured_at` is null while `lag` is still returned. **The reset used to delete the window between runs and no longer does (WO-R3-333, [ADR 0038](ADR/0038-a-probe-by-the-lab-is-labelled-by-the-lab.md)).** The reasoning for deleting it was that a trend measured in one run must not be the first thing the next run sees; the cost showed up on the demo's third live take, where the operator opened a freshly reset world and the fifteen-minute chart held two points while the fault it exists to show was climbing 0 → 10 → 30. The window is history, and its TTL is longer than the value key's *because* history is most wanted when the pass that writes it stopped — deleting it on the boundary contradicted the reason it is kept. The value key is untouched in shape and meaning: loop-owned, fresh-or-absent, which is what `check_backpressure` needs — only its TTL moved, from a fixed 90s to three passes (WO-R3-338). `make eval-reset` still reports `lag_samples_cleared`, permanently `0`, because the summary is also the `lab.world_reset` row's payload and a counter that disappears reads as a step that stopped being reported. A consumer of the window that must not read across a reset has the boundary row's timestamp to compare each sample against.
