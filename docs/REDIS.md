...
| `chaos:downstream:bulk_api_sync` | string (`{mode}:{delay_ms}`) | `degrade_downstream` chaos tool (`downstream_flag_key` in `app/workers/async_tasks.py`) | `process_bulk_api_sync`, once per job — so a whole fan-out sees one dependency state. `fail` makes every endpoint return 503 and the shipped breaker opens at its own threshold; `slow` makes each answer late and succeed. While set, a job whose every endpoint call failed is itself failed (ADR 0031) | `ttl_seconds` (default 300s, max 3600s) | Yes — calls go back to normal | Yes (`chaos:*` scan); dead-lettered jobs left behind are organic DLQ rows, swept by `_sweep_nonfixture_dlq` |
