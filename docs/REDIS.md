...
| `kafka:consumer_lag:worker-dispatcher:samples` | string (JSON list, newest first, max 5 × `{lag, measured_at}`) | `dispatcher.py` `_record_lag_sample`, from the same loop pass that writes the value above | `get_consumer_lag` MCP tool (`recent_samples` / `measured_at` / `age_seconds`) | 90s, refreshed on every write | Yes — the tool reports an absent window as no history, never as a flat lag |
