...
| `kafka:consumer_lag:{group}:samples` | string (JSON list, newest first, max 15 × `{lag, measured_at}` — 15 minutes at one per pass) | `core/consumer_lag.record_lag_sample`, called by `dispatcher.py` `_record_lag_sample` from the same loop pass that writes the value above. Keyed by group; only `worker-dispatcher` has a writer | `get_consumer_lag` MCP tool (`recent_samples` / `measured_at` / `age_seconds`) and `GET /admin/consumer-lag` | **1080s** (the 900s window plus 180s), refreshed on every write | Yes — the reader reports an absent window as no history, never as a flat lag |
