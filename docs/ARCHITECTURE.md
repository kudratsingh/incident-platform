...
| `LlmTriageConsumer` | `job.dlq` | `job_triages` rows + `jobs.remediation_hint` | Per-job — an LLM failure is logged, no row is written, and the offset is committed ([ADR 0005](ADR/0005-llm-features-fail-open.md)). Only 429/5xx re-raise for redelivery; anything deterministic would otherwise loop on a billed call. The hint is derived from the same analysis and written in the same transaction, only into a NULL column (R2-24) |
...
Separately from failure, each of the eleven loops can be **deliberately stopped one at a time** in a lab environment: `worker_loop` starts eleven, every one of them reads `chaos:pause:<loop>` once per iteration, and the `pause_control_loop` chaos tool sets that key with a TTL ([ADR 0027](ADR/0027-control-loop-pause-closed-enum.md)). The check short-circuits on `CHAOS_ENABLED` before any Redis call, so outside a lab it is one boolean per loop per tick. The eight consumer groups are stopped by `kill_consumer` instead, which is why they are not in that enum. Note the table above lists the eight loops whose failure behaviour differs; `_promote_dlq_replay_loop`, `_resume_unblocked_waiting_loop` and `_idempotency_reaper_loop` follow the same per-tick pattern (log and keep turning) and `worker_loop`'s own docstring is the authority on the full set.

Two paths still escape those top-level guards — a loop's deferred imports, which sit *before* its `while True`, and a `CancelledError` reaching `_supervise_consumer`, which re-raises by design. Both end `worker_loop` itself, so the task is supervised in turn by `app/workers/supervisor.py`: the death is logged, the worker is restarted (immediately, then 1s → 30s), and its liveness is reported on `GET /healthz/worker` (and, for operators, on `GET /api/v1/health`). `_promote_delayed_loop` also calls `worker_tick()` on every pass, which extends that liveness from "the task exists" to "its loops are turning". This matters more than the per-loop isolation, because `ConsumerLag` — the metric both backlog alarms read — is emitted by `_metrics_loop`, a loop inside the worker, and is absent rather than high when the worker is dead. Without the health signal, worker death silences the metrics that would detect it.
...
**Detection:** `GET /healthz/worker` reports `"worker": "error"` and 503 — the ECS container check probes it, so a worker that cannot be restarted in-process gets the task recycled. `GET /api/v1/health` reports the same thing alongside Postgres and Redis, with a `worker_detail` giving the state, restart count and last error; nothing automatic acts on that endpoint (WO-R2-65). Do **not** wait on the backlog alarm for this one: `ConsumerLag` is emitted by a loop inside the worker and is not emitted at all when lag is unknown, so a dead worker makes the datapoints *absent* and the alarm reads missing data as `notBreaching` — `infra/cloudwatch.tf` hands this case to worker supervision explicitly. `backend-tasks-low` only fires if the whole ECS task drops, which a dead worker task inside a live API process does not do.
...
**What a failed job means here:** a job whose *every* endpoint call failed is a failed job — it retries, and then dead-letters with `bulk api sync failed: all N endpoint calls failed (0 of N endpoints returned a result)` on the row. That is unconditional platform behaviour, not something the lab switches on (WO-R3-322, owner decision O-31 D4; [ADR 0031](ADR/0031-a-held-pool-and-a-degraded-dependency-are-flagged-not-broken.md)'s 2026-09-19 amendment). A *partial* failure still completes, with the per-endpoint errors counted in the job's result payload. The SLO that moves when such a job dies is `job_completion_rate`; the retries on the way spend no budget.
...
**In the eval world:** `degrade_downstream` sets one key that makes the simulated endpoint calls fail (`mode=fail`, which the breaker counts) or answer late but succeed (`mode=slow`, which it does not). The flag chooses the injection only — the failed-job rule above applies to the organic 10%-per-endpoint path just the same, so `search_traces(job_type="bulk_api_sync", status="failed")` has rows to find either way. See [ADR 0031](ADR/0031-a-held-pool-and-a-degraded-dependency-are-flagged-not-broken.md) and its 2026-09-19 amendment.
...
**`SLO_EVALUATION_INTERVAL_SECONDS=0` is also the eval world's workaround, and the reason is worth stating.** The seeded eval fixtures are an SLO breach by construction: 4 of the 7 seeded jobs are the DLQ fixtures, so `job_completion_rate` reads 42.9% against a 99% objective the moment the stack finishes booting — far past 14.4×, with no traffic and nothing running. A freshly booted eval world therefore alerts on itself within one interval, and because the dedup key is hour-bucketed (above), it re-fires once per UTC hour for as long as the stack lives; `scripts/reset_eval_state.py` does not clear it, because that sweep only matches `source LIKE 'chaos:%'` and these carry `slo:<id>` (WO-R2-131). The result is a permanent `critical` distractor in every scenario's world — first observed live on 2026-09-07 against a fresh v0.6.0 demo stack. The eval compose sets the interval to `0` to suppress it. That is a mitigation, not the fix: it also removes the evaluator from the surface the agent is being graded on. The durable fix is to exclude fixture-seeded rows from the SLI denominators, so the eval world looks like production with the fixtures invisible to the objectives — **WO-R2-132**.
...
- `backend/app/workers/supervisor.py` — supervises that task; the liveness `/healthz/worker` reports
