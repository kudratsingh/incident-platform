...
- **What it serves** — 23 tools by default: 16 read tools (consumer lag, outbox delivery, DLQ contents, traces, deploy history, DAG state, Redis/Postgres health, circuit breakers, SLO status, incidents, alerts, audit events, cache key info) and 7 Tier-1 action tools (`restart_consumer_group`, `replay_dlq_messages`, `replay_dlq_by_ids`, `replay_dlq_by_category`, `pause_dag`, `invalidate_cache_key`, `mark_dlq_permanent`). With `CHAOS_ENABLED=true` a further 14 lab hooks register, for a total of 37 — they are never registered in production ([ADR 0008](docs/ADR/0008-chaos-gating.md)). Count them with `list_tools()`: this line has been stale before.
