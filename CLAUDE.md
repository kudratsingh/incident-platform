...
- **Don't cut a tag before the clean-baseline rerun.** `master` currently carries the deferred rule-2 work from [ADR 0012](docs/ADR/0012-the-lab-is-invisible-to-the-agent.md) — inert (`EVAL_EMPTY_DLQ_BASELINE` defaults off) but it registers one extra chaos tool, so `master` serves 27 tools where `v0.4.9` serves 26. Tagging would put that in an image and force an agent-side contract re-sync. Merging was never the risk; releasing is.
