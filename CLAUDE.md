...
- [`docs/ADR/`](docs/ADR/) — architecture decision records. Read these to understand *why* the platform looks the way it does. [`docs/ADR/README.md`](docs/ADR/README.md) is the full index with every status; 38 of the 39 are listed below (0034 is in the index only):
  ...
  - [0012 — The lab is invisible to the agent](docs/ADR/0012-the-lab-is-invisible-to-the-agent.md) — rule 1 shipped v0.4.9 and now covers response bodies as well as descriptions (2026-09-15 amendment); rule 2 deferred to post-rerun; the withheld audit set is **two** prefixes since the 2026-09-20 amendment — `chaos.` (a fault going in) and `lab.` (the environment reset's `lab.world_reset` boundary row, whose payload is the reset's counters and therefore the mechanism list), both under the one `chaos:invoke` condition. A second prefix rather than `chaos.world_reset` because to the `/demo` console the newest `chaos.*` row *is* the fault, so a boundary filed there would read as one. A **second 2026-09-20 amendment** (WO-R3-333) covers the case rule 1 never faced — the lab wearing the *agent's* identity, which the principal guards and the world audit have to do — so `lab.` gained a second member, `lab.probe`, under the same one condition ([ADR 0038](docs/ADR/0038-a-probe-by-the-lab-is-labelled-by-the-lab.md))
  ...
  - [0038 — A probe the lab makes on the agent's token is labelled by the lab, and the label needs the lab's own credential](docs/ADR/0038-a-probe-by-the-lab-is-labelled-by-the-lab.md) — the demo's third take, platform half (WO-R3-333). Two evaluator callers wear the AGENT's token on purpose, because that token is what they are asserting about: the principal guards (a Tier-1 and a chaos attempt that must both be refused) and the world audit (reads taken exactly as the agent would take them). Every one of those calls wrote `agent.tool_invoked`, so the `/demo` ledger showed seven reads the agent never made. `tools/call` now takes `_lab_probe` **in `params`, beside `arguments` and never inside it** — inside, it would reach a tool's input model and therefore `tools/list`, which is the contract the commander pins and the surface the planner reads; beside it, `tools/list` is byte-identical and there is nothing to rebless. The row becomes `lab.probe`, which puts it in the stream already withheld from the agent and keeps `agent.tool_invoked` meaning what the agent did. The half that took the thinking is that a label the subject under test can apply to its own reads is worse than the mislabel it fixes, so it needs a second credential — `X-Lab-Principal`, holding `chaos:invoke` or the read-only smoke account *by name* (that account holds the agent's scopes exactly, which is the point of it, so nothing else tells them apart) in the caller's own tenant on a `CHAOS_ENABLED` stack — and an unauthorised field is **refused, never ignored**: an ignore leaves the row mislabelled *and* the caller believing otherwise. The label replaces `agent.tool_invoked` and nothing else, because a `chaos.` row is what the console reads as the fault. Second half, same take: the environment reset stops deleting the 15-minute lag window — it is history, its TTL is longer than the value key's precisely so it outlives the pass that wrote it ([ADR 0037](docs/ADR/0037-a-run-record-carries-the-run.md)), and deleting it on the boundary opened the demo's chart on two points while the fault it is drawn to show was climbing. `lag_samples_cleared` stays in the summary as a permanent 0, because that summary is also the boundary row's payload
  - [0039 — The platform pages on its own metric, and an episode raises once](docs/ADR/0039-the-platform-pages-on-its-own-metric.md) — the demo's fourth take, platform half (WO-R3-338, owner decisions O-35 and O-36). Two things the take exposed, one of them about honesty and one about speed. The alert the responder triaged was written by the scenario file that ran it, and the platform's own stream read the same three seeded fixtures before, during and after the fault — the demo's middle clause, "the platform pages", was a fixture. And the lag clock was a module constant at 60 s, so the number an operator stares at moved once a minute. So the interval becomes a setting (`METRICS_LOOP_INTERVAL_SECONDS`, default 60, **5 in the demo stack**) and the two numbers that were literals beside it are derived from it: the value key's TTL is three passes rather than a fixed 90 s (1.5 passes at 60 s, eighteen at 5 s — and that key's whole contract is fresh-or-absent), and the fifteen-minute window is pruned by TIME rather than by a count of passes, so the span is the promise and the density follows the clock (15 samples at 60 s, 180 at 5 s, an absolute 240 guard that binds below 3.75 s) — the OPERATOR's `GET /admin/consumer-lag` serves that whole window while the AGENT's `recent_samples` stays capped at the newest 15 it always returned, since a reading whose size changed with a deployment's tick would cost one stack's investigation more context than another's for no new information. Then two rules on that same tick: `consumer_stalled` on the latest **measured** sample — never on the seven recorded constants, which nothing can bring down — and `dlq_depth_warning` on a tenant's dead-letter total at the seeded baseline plus one. Each raises ONCE per episode through `dedup_key = rule:<fingerprint>:<subject>:<ordinal>`, the ordinal counted from rows so two replicas build the same key and the unique constraint settles it; a Redis episode marker was rejected as one more piece of state the reset must sweep, whose failure mode is that the NEXT take never pages, silently. An absent reading is neither a breach nor a recovery, because unknown is not zero. `alert.raised` / `alert.resolved` carry the page for the console, withheld from nobody. One contract delta — `get_consumer_lag`'s description had "every ~60s" and "90s TTL" in it and would have been lying on the one stack anybody watches; it now names no interval, points at `age_seconds` and states the fifteen-sample cap, and it must not interpolate the setting or the snapshot the commander pins would differ per deployment. No migration, no new table, column, key, scope or refusal code
...
- **Eleven background loops** also running in the same process:
  ...
  - **Metrics loop** — emits CloudWatch gauges (`QueueDepth`, `InFlightJobs`, `ConsumerLag`), caches the lag in Redis for the backpressure check, records it in the fifteen-minute window the `/demo` chart reads, and since WO-R3-338 evaluates the platform's own two alert rules on the same tick, right after the measurement they read ([ADR 0039](docs/ADR/0039-the-platform-pages-on-its-own-metric.md)). Its interval is `METRICS_LOOP_INTERVAL_SECONDS` (default 60, **5 in the demo stack**), read every pass and clamped in one place, and both the value key's TTL and the window's density are derived from it
...
- `lab.probe` — the fifth machine stream, and the only one that is an ordinary tool call wearing a different name (WO-R3-333, [ADR 0038](docs/ADR/0038-a-probe-by-the-lab-is-labelled-by-the-lab.md)). One MCP call the lab made **under the agent's own token**, which the principal guards (`evals/guards.py`) and the world audit (`evals/world_audit.py`) have to do because that token is the thing they are asserting about. It is written when `tools/call`'s `params` carries `_lab_probe` — a short reason string, **beside `arguments` and never inside it**, so no tool's input model sees it and `tools/list` does not move — and only when the request also carries `X-Lab-Principal: Bearer <token>` verifying to a principal that holds `chaos:invoke` or is the read-only smoke account (`lab_probe_smoke_account_name`, matched by name because it holds the agent's scopes exactly, and re-checked to hold no write scope), in the caller's own tenant, on a `CHAOS_ENABLED` stack. Anything else is a JSON-RPC invalid-params refusal (`error_code: lab_probe_refused` plus a closed `reason_code`) and **the call does not run**: a silent ignore would leave the row saying `agent.tool_invoked` while the caller believed otherwise. Same row shape as `agent.tool_invoked` plus `extra_data.lab_probe_reason` (≤ 200 characters, refused rather than truncated) and `extra_data.lab_probe_principal`. It replaces `agent.tool_invoked` **and nothing else** — a chaos row or a run report keeps its action and carries the reason in `extra_data`, because to the `/demo` console the newest `chaos.*` row *is* the fault. **Withheld from any principal without `chaos:invoke`**, by the `lab.` prefix rule the boundary row above already established: a read the agent did not make must not come back to it as its own. Not a tool, so nothing in `tools/list` moves and there is nothing to rebless. The request shape is in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md#_lab_probe-a-call-the-lab-makes-on-the-agents-token-says-so-wo-r3-333).
- `alert.raised` / `alert.resolved` — the sixth stream, and the only one **withheld from nobody** (WO-R3-338, [ADR 0039](docs/ADR/0039-the-platform-pages-on-its-own-metric.md)). One row per transition of an alert the platform raised on its own metric — `consumer_stalled` on the latest measured consumer-lag sample, `dlq_depth_warning` on a tenant's dead-letter total, both evaluated in the metrics pass. `resource_type: alert`, `resource_id` the alert id, `extra_data` carrying the fingerprint, subject, source, severity, one-sentence summary and `dedup_key` (plus `resolved_reason` on a resolve) — enough for the `/demo` console to draw a "platform paged" station from the audit stream alone. `principal_type='service_account'` with a **null `principal_id`**: no human and no account did this, the platform's own loop did, and [ADR 0007](docs/ADR/0007-machine-principal-scope-model.md) made that id nullable so an actor without one still gets a row. It is readable by every principal on purpose — [ADR 0012](docs/ADR/0012-the-lab-is-invisible-to-the-agent.md) hides the lab and the inverse rule hides a responder's own reports from its writer, and an alert is the opposite kind of row: it is the thing the agent was *paged with*, raised by a documented rule over a reading the agent can take itself, naming no mechanism. Written in the alert's own transaction and **not** savepoint-wrapped: `record_world_reset`'s contract, for its reason — there is no response to protect, and a page the console cannot see is worse than a page one tick late. Not a tool, so nothing in `tools/list` moves for it.
